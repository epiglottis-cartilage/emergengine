...
use std::{cell::UnsafeCell, collections::HashMap, path::Path, sync::Arc};
...
pub struct Primitive {
    ...
    pub index_buf: Option<(wgpu::Buffer, u32)>,
    pub topo: wgpu::PrimitiveTopology,
...
impl Model {
    pub fn load<P>(path: P, device: &wgpu::Device, queue: &wgpu::Queue) -> Result<Vec<Self>>
    where
        P: AsRef<Path>,
    {
        let (document, buffers, images) = gltf::import(path).log()?;
        ModelLoader::new(device, queue, document, buffers, images).load()
    }
}
...
impl<'a> ModelLoader<'a> {
    ...
    fn load_node(&self, node: gltf::Node, transform: Mat4) -> Result<Node> {
        log::debug!("Loading node {:?}", node.name());
        let children = node
            .children()
            .map(|n| {
                let transform = transform * Mat4::from_cols_array_2d(&n.transform().matrix());
                self.load_node(n, transform)
            })
            .try_collect()?;
        let mesh = match node.mesh() {
            None => None,
            Some(m) => Some(self.load_mesh(m)?),
        };
        Ok(Node {
    ...
    fn load_mesh(&self, mesh: gltf::Mesh) -> Result<Mesh> {
...
pub trait DrawModel<'a> {
    fn draw_mesh(&mut self, mesh: &'a Primitive, instances: Range<u32>);
}
impl<'a, 'b> DrawModel<'b> for wgpu::RenderPass<'a>
where
    'b: 'a,
{
    fn draw_mesh(&mut self, mesh: &'b Primitive, instances: Range<u32>) {
        match &mesh.index_buf {
            None => {
                todo!()
            }
            Some((index_buf, len)) => {
                // self.set_bind_group(0, &self.diffuse_bind_group, &[]);
